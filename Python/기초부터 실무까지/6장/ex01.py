...
say_hello_name("김연아")
say_hello_name("홍길동")
# 함수가 호출되어 10을 출력하긴 하지만 가독성이 좋지않다.
# 이유는 함수의 매개변수명이 name이니 웨만하면 이름을 매개변수 값으로 주는 것이 현명한 코드가 될 것이다.
# say_hello(10)

# 파이썬에서는 오버로딩의 개념이 없다. 같은 함수의 이름이라면 마지막에 정의되어진 함수만 인식하게 된다.
# 하여, 함수명은 유니크한 값으로 함수명을 짓도록 한다.

say_hello_name_msg("김연아", "반갑습니다.")
say_hello_name_msg("홍길동", "도와주세요.")

# get_sum을 이용하여 범위값의 누적합을 구하는 코드
result = get_sum(1, 10)
print(type(result))
